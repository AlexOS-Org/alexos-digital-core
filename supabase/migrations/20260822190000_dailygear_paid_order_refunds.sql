-- Controlled paid-order reversal. Never hard-delete order or payment evidence.
-- `void` removes the linked income from active balances by voiding it.
-- `refund` additionally records one customer-refund expense from the selected account.
...
BEGIN
  ...
  SELECT p.id, p.amount, p.money_transaction_id, p.transaction_id INTO v_payment
    FROM public.dg_order_payments p
    WHERE p.order_id = p_order_id AND p.user_id = v_user_id
  ...
  IF p_mode = 'refund' THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.accounts a WHERE a.id = p_refund_account_id AND a.user_id = v_user_id AND a.deleted_at IS NULL) THEN
      RAISE EXCEPTION 'Refund account not found';
    END IF;
    v_reference := 'DG_REFUND:' || v_order.order_number || ':' || trim(p_refund_transaction_id);
    IF EXISTS (SELECT 1 FROM public.transactions t WHERE t.user_id = v_user_id AND t.reference = v_reference) THEN
      RAISE EXCEPTION 'Refund reference already used';
    END IF;
    INSERT INTO public.transactions (
      user_id, account_id, type, amount, occurred_at, category, source,
      description, reference, status, financial_scope, business_name, flow_type
    ) VALUES (
      v_user_id, p_refund_account_id, 'expense', v_amount, now(), 'Customer refund', 'DailyGear',
      'Refund for ' || v_order.order_number || COALESCE(' · ' || p_notes, ''), v_reference,
      'posted', 'business', 'DailyGear', 'standard'
    ) RETURNING id INTO v_refund_tx;
  END IF;

  UPDATE public.transactions
    SET status = 'void', deleted_at = COALESCE(deleted_at, now())
    WHERE id = v_payment.money_transaction_id AND user_id = v_user_id AND status = 'posted';
  ...
  VALUES (v_user_id, p_order_id,
    CASE WHEN p_mode = 'refund' THEN 'refund' ELSE 'payment' END,
    CASE WHEN p_mode = 'refund' THEN 'Payment refunded' ELSE 'Payment voided' END,
    concat('KES ', to_char(v_amount, 'FM999999990.00'), ' ', p_mode, ' · original transaction ', v_payment.transaction_id));
