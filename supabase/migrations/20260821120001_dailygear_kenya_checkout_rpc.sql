-- Kenya-first extension of the existing atomic DailyGear guest-order RPC.
-- The storefront and funnel still share this one checkout and order pipeline.

drop function if exists public.dg_create_guest_order(text,text,text,text,text,text,text,text,text,jsonb);
drop function if exists public.dg_create_guest_order(text,text,text,text,text,text,text,text,text,jsonb,uuid,jsonb);
