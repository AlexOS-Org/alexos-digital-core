-- Fix the production guest-order RPC error:
-- record "v_variant" is not assigned yet.
--
-- The previous function referenced fields on an unassigned PL/pgSQL record in
-- the JSON line builder when a cart line did not carry a variant. This version
-- uses nullable scalar fields and resets them for every line. It preserves the
-- single atomic checkout path, catalogue gates, payment-state semantics and
-- order-event writes. Quantity is informational; the owner controls sellability
-- by setting the product status to out_of_stock.
...
begin
  ...
  loop
    ...
    -- Reset nullable variant fields for every line. A parent-product line must
    -- never read a field from a record that was not selected.
    v_variant_name := null;
    v_variant_sku := null;
    v_variant_image_url := null;
    v_variant_price := null;
    v_variant_sale_price := null;
    v_variant_cost_price := null;
    v_variant_stock_quantity := null;
    ...
    if v_item.variant_id is not null then
      ...
      into
        ...
        v_variant_image_url,
        v_variant_price,
        v_variant_sale_price,
        v_variant_cost_price,
        v_variant_stock_quantity
      ...
      if v_variant_sale_price is not null
         and v_variant_sale_price > 0
         and v_variant_sale_price < v_unit_price then
        v_unit_price := v_variant_sale_price;
      end if;
      v_unit_cost := coalesce(v_variant_cost_price, v_product.cost_price, 0);
    ...
    v_lines := v_lines || jsonb_build_array(jsonb_build_object(
      ...
      'name', case
        when v_item.variant_id is null then v_product.name
        else v_product.name || ' — ' || v_variant_name
      end,
      'sku', case
        when v_item.variant_id is null then v_product.sku
        else coalesce(v_variant_sku, v_product.sku)
      end,
      'quantity', v_item.quantity,
      'unit_price', v_unit_price,
      'unit_cost', v_unit_cost,
      'image_url', v_variant_image_url,
  ...
  -- The owner-managed stock quantity is informational for this catalogue.
  -- It is not decremented or used as a checkout cap. The owner can set the
  -- product to out_of_stock when it should stop accepting orders.
