-- DailyGear availability is owner-controlled. Quantity is informational and must not
-- prevent publication or order capture. The owner can stop sales explicitly with
-- dg_product_status = 'out_of_stock'. Evidence, category, SEO and media gates remain.
...
begin
  ...
  if new.status = 'active' and not exists (
    select 1 from public.dg_product_evidence e
    where e.product_id = new.id and e.reconciliation_status = 'verified'
  ) then
  ...
  if new.status = 'active' and coalesce(new.availability_confirmed, false) = false then
    raise exception 'An active product must have confirmed availability.';
  end if;

  if new.status = 'active' and exists (
    select 1 from public.dg_product_variants v
    where v.product_id = new.id and v.deleted_at is null
      and coalesce(v.availability_confirmed, false) = false
  ) then
    raise exception 'Every published colour or SKU variant must have confirmed availability.';
  end if;
...
begin
  select status into parent_status from public.dg_products
  where id = new.product_id and deleted_at is null;

  if parent_status = 'active' and coalesce(new.availability_confirmed, false) = false then
    raise exception 'Every published colour or SKU variant must have confirmed availability.';
  end if;
