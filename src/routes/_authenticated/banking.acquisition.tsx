    ...
    <div className="mx-auto max-w-7xl space-y-6">
          ...
          onSubmit={async (event) => {
            ...
            try {
              await createEmployer.mutateAsync({
                ...
                recruitment_frequency: parseRecruitmentFrequency(data.get("frequency")),
                hiring_momentum_score: Math.max(0, Math.min(100, Number(data.get("score")) || 50)),
                priority: parseEmployerPriority(data.get("priority")),
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border bg-card">
          <div className="border-b p-4">
            <h2 className="font-semibold">Employers to target</h2>
            <p className="text-xs text-muted-foreground">Prioritised by hiring momentum.</p>
          </div>
              ...
              <div key={employer.id} className="flex items-center justify-between gap-4 p-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {employer.industry || "Industry not set"} · {employer.location || "Nairobi"}
                  </p>
                </div>
                <div className="text-right">
                  <p className="font-semibold">{employer.hiring_momentum_score}</p>
                  <p className="text-[10px] uppercase text-muted-foreground">momentum</p>
                </div>
              </div>
            ))}
            {!employers.data?.length && (
              <Empty text="No employers yet. Add the first company you know recruits regularly." />
            )}
          </div>
        </section>
        ...
        <section className="rounded-2xl border bg-card">
          <div className="border-b p-4">
            <h2 className="font-semibold">Hiring signals</h2>
            <p className="text-xs text-muted-foreground">Vacancy and recruitment triggers.</p>
          </div>
          <div className="divide-y">
            {(signals.data ?? []).slice(0, 10).map((signal) => (
              <div key={signal.id} className="p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {signal.signal_type} · {signal.vacancy_count} vacancies · ~
                  {signal.estimated_hires} hires
                </p>
              </div>
            ))}
            {!signals.data?.length && (
              <Empty text="No signals yet. The next phase will ingest recruitment sources." />
            )}
          </div>
        </section>
      </div>
      ...
      <section className="rounded-2xl border bg-card">
        <div className="flex items-center justify-between border-b p-4">
          <div>
            <h2 className="font-semibold">Employee pipeline</h2>
            <p className="text-xs text-muted-foreground">
              Banking acquisition prospects are isolated until linked to CRM.
            </p>
          </div>
          ...
          {(prospects.data ?? []).map((prospect) => (
            <div key={prospect.id} className="flex items-center justify-between gap-4 p-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {prospect.job_title || "Role not set"} · {prospect.stage.replaceAll("_", " ")}
                </p>
              </div>
              {prospect.crm_contact_id ? (
                ...
                <button
                  type="button"
                  onClick={() => linkToCrm(prospect)}
                  disabled={linkingId === prospect.id}
                  className="rounded-lg border px-3 py-1.5 text-xs hover:bg-accent"
                >
                  {linkingId === prospect.id ? "Linking…" : "Add to CRM"}
                </button>
              )}
            </div>
          ))}
      ...
      <p className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-xs text-muted-foreground">
        Credit and other product opportunities are tracked as signals only. KYC, affordability,
        credit scoring, consent and final lending decisions remain with the bank's approved systems
        and policies.
      </p>
    </div>
  );
