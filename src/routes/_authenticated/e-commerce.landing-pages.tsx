              ...
              {products.map((product) => (
                ...
                >
                  ...
                  <p className="mt-3 line-clamp-3 text-sm text-muted-foreground">
                    {product.description ??
                      "No product description yet. The campaign preview will use conversion-focused fallback copy."}
                  </p>
      ...
      <section id="landing-preview" className="scroll-mt-6">
        {selectedProduct ? (
          <LandingPagePreview
            ...
            onOpenCheckout={() => navigate({ to: "/shop/cart" })}
          />
        ) : (
          <DemoLandingPage />
        )}
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-8 sm:px-8 lg:grid-cols-[1fr_.9fr] lg:items-center lg:gap-14 lg:py-14">
          <div className="order-2 lg:order-1">
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Button
                size="lg"
                className="rounded-xl bg-white px-7 text-zinc-950 hover:bg-zinc-100"
                disabled={soldOut}
                onClick={addToCart}
              >
          ...
          <div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {isInverter ? (
                <>
                  <Benefit
                    title="Charge more on the road"
                    text="Power compatible small electronics from your vehicle when travelling, working remotely or waiting between appointments."
                  />
                  <Benefit
                    title="Multiple connection options"
                    text="Use AC outlets and USB charging ports to keep everyday devices connected from one compact unit."
                  />
                  <Benefit
                    title="Designed for vehicle use"
                    text="A compact plug-in design keeps the inverter close at hand without taking over your dashboard or cabin."
                  />
                  <Benefit
                    title="Protection built in"
                    text="Multiple protection features help guard the inverter and connected equipment against common electrical faults."
                  />
                </>
              ) : (
                <>
                  <Benefit
                    ...
                    text="Focused on a real everyday need, with simple controls and a straightforward customer experience."
                  />
                  <Benefit
                    title="Made for daily use"
                    text="A useful addition to your routine, whether you are at home, at work or on the move."
                  />
                  <Benefit
                    title="Easy to order"
                    text="Choose your product, add it to your bag and complete the simple DailyGear checkout."
                  />
                  <Benefit
                    title="Delivery across Kenya"
                    text="DailyGear is built around clear delivery information and a customer-first ordering flow."
                  />
                </>
              )}
            </div>
            ...
            <div className="mt-10 rounded-3xl border bg-muted/30 p-6">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {(isInverter
                  ...
                  : [
                      "Quality DailyGear product",
                      "Simple ordering experience",
                      "Secure checkout",
                      "Kenya delivery support",
                      "Product support",
                      "Clear pricing",
                    ]
                ).map((item) => (
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              <TrustCard
                ...
                text="Clear delivery details at checkout."
              />
              <TrustCard
                icon={<ShieldCheck />}
                title="Secure checkout"
                text="Your order details are handled securely."
              />
              <TrustCard
                icon={<Clock3 />}
                title="Quick ordering"
                text="Less friction between interest and purchase."
              />
            </div>
          </div>
      ...
      <div className="mx-auto max-w-5xl px-5 py-12 text-center sm:px-8 lg:py-20">
        ...
        <div className="mx-auto mt-8 grid max-w-3xl gap-3 text-left sm:grid-cols-2">
          {items.map((item) => (
            <div
              key={item}
              className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-zinc-200"
            >
