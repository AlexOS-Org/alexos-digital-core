...
export const Route = createFileRoute("/_authenticated/e-commerce/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Revenue, profit, order and inventory KPIs for your DailyGear commerce operation." },
      { property: "og:title", content: "DailyGear Commerce Overview | Alex OS" },
      { property: "og:description", content: "Revenue, profit, order and inventory KPIs for your commerce operation." },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Revenue (30d)" value={money(kpis.revenue)} icon={DollarSign} changePct={kpis.revenueChangePct} loading={isLoading} />
        <KpiCard label="Gross profit" value={money(kpis.profit)} icon={TrendingUp} tone="positive" loading={isLoading} />
        <KpiCard label="Orders (30d)" value={kpis.orders} icon={ShoppingCart} changePct={kpis.ordersChangePct} hint={`${kpis.pendingOrders} awaiting fulfilment`} loading={isLoading} />
        <KpiCard label="Inventory value" value={money(kpis.inventoryValue)} icon={Boxes} tone={kpis.lowStockCount ? "warning" : "default"} hint={`${kpis.lowStockCount} low-stock items`} loading={isLoading} />
      </div>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Average order value" value={money(kpis.averageOrderValue)} icon={DollarSign} loading={isLoading} />
        <KpiCard label="Customers" value={kpis.customers} icon={Users} hint={`${kpis.returningCustomers} returning`} loading={isLoading} />
        <KpiCard label="Delivered orders" value={kpis.deliveredOrders} icon={ShoppingCart} tone="positive" loading={isLoading} />
        <KpiCard label="Purchase rate" value={`${kpis.conversionRate.toFixed(0)}%`} icon={TrendingUp} hint="Customers who have ordered" loading={isLoading} />
      </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {trend.map((point) => (
              <div key={point.label} className="rounded-xl border border-border/70 p-3">
                ...
                <p className="text-xs text-muted-foreground">{point.orders} orders · {money(point.profit)} profit</p>
              </div>
            ))}
          </div>
        </CardContent>
