      ...
      return (
        <Card className="rounded-3xl border-amber-500/20 bg-amber-500/5">
          <CardContent className="p-5">
            <p className="text-sm font-semibold">Orion is still bringing this panel into focus.</p>
            <p className="mt-1 text-xs text-muted-foreground">The rest of your command center is available.</p>
          </CardContent>
        </Card>
      );
    }
  ...
  return (
    <div className="relative space-y-10 pb-10 animate-in fade-in duration-500">
      ...
      <SafePanel>
        <section className="relative overflow-hidden rounded-3xl border border-primary/15 bg-gradient-to-r from-primary/[0.07] via-background to-[var(--orion-purple)]/[0.07] p-5 sm:p-6">
          ...
          <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="w-full lg:max-w-xl"><AskAlexBar /></div>
          </div>
        </section>
      </SafePanel>

      <SafePanel><AIBriefing /></SafePanel>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <SafePanel><IntelligenceFeed /></SafePanel>
      </section>
        ...
        <section className="space-y-5">
          ...
          <Tabs defaultValue={moduleGroups[0]} className="space-y-5">
            <TabsList className="flex h-auto w-full justify-start gap-1 overflow-x-auto rounded-2xl border border-border/60 bg-muted/50 p-1">
              {moduleGroups.map((group) => <TabsTrigger key={group} value={group} className="rounded-xl px-4 py-2">{group}</TabsTrigger>)}
            </TabsList>
                    ...
                    <Link key={m.url} to={m.url} className="group">
                      <Card className="h-full rounded-3xl border-border/60 bg-card/80 transition-all duration-200 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5">
                        <CardContent className="space-y-4 p-5">
                          ...
                          <span className="inline-flex rounded-full border border-border/60 bg-muted/60 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{m.group}</span>
                        </CardContent>
                      </Card>
                    </Link>
                  ))}
