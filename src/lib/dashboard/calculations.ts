...
export function computeMoneyMetrics(snapshot: DashboardSnapshot, now = new Date()): MoneyMetrics {
  ...
  const lowBalanceAccounts = accounts
    .map((account) => {
      ...
      return {
        ...
        flagged: threshold !== null && balance < threshold,
      };
    })
    .filter((a) => a.flagged)
  ...
  const thisMonthTx = transactions.filter((t) =>
    inMonth(new Date(t.occurred_at), now.getFullYear(), now.getMonth()),
  );
  const lastMonthTx = transactions.filter((t) =>
    inMonth(new Date(t.occurred_at), prev.getFullYear(), prev.getMonth()),
  );
  ...
  const billsDueSoon = unpaidBills.filter((b) => {
    ...
    const diff = Math.round(
      (new Date(`${b.due_date}T00:00:00`).getTime() - new Date(todayKey + "T00:00:00").getTime()) /
        DAY_MS,
    );
  ...
  return {
    ...
    savingsRate:
      incomeThisMonth > 0
        ? Math.max(0, Math.min(100, (netFlowThisMonth / incomeThisMonth) * 100))
        : 0,
    revenueToday,
...
export function computeBusinessMetrics(
  ...
  return {
    ...
    weightedPipelineValue: open.reduce(
      (sum, l) => sum + (num(l.value) * num(l.probability)) / 100,
      0,
    ),
...
export function computeGoalMetrics(snapshot: DashboardSnapshot, now = new Date()): GoalMetrics {
  ...
  const saved = (goalId: string) =>
    num(snapshot.goalProgress.find((p) => p.goal_id === goalId)?.current_amount);
