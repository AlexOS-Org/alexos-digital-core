...
export function buildAurenDataReadiness(
  ...
  feeds.push({
    ...
    detail:
      tx > 0
        ? `${tx.toLocaleString()} rows in scope`
        : "Next step: post at least one income or expense.",
  });
  ...
  feeds.push({
    ...
    detail:
      leads > 0
        ? `${leads.toLocaleString()} lead row(s)`
        : "Next step: create at least one lead with a stage.",
  });
