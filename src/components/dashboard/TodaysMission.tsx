      ...
      <div className="relative p-6 sm:p-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Ranked from your live money, pipeline and goal data.</p>
          </div>
          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-xs text-slate-300">
            <Zap className="h-3.5 w-3.5 text-violet-300" /> {isLoading ? "Calculating" : `${activeCount} active priorities`}
          </div>
        </div>

        {isError ? (
          <p className="mt-7 rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm text-slate-300">Priorities are unavailable right now. Refresh to retry.</p>
        ) : (
          <div className="mt-7 grid gap-3 lg:grid-cols-3">
            {isLoading
              ? Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-[140px] rounded-2xl bg-white/[0.06]" />)
              : priorities.map((task, index) => (
                  <Link key={task.id} to={task.to} className="rounded-2xl border border-white/10 bg-white/[0.045] p-4 backdrop-blur-sm transition-colors hover:bg-white/[0.07]">
                    <div className="flex items-center justify-between gap-3">
                      <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${task.count > 0 ? TONES[task.tone] : "bg-emerald-400/10 text-emerald-300"}`}>
                        {task.count > 0 ? <Clock3 className="h-5 w-5" /> : <CheckCircle2 className="h-5 w-5" />}
                      </div>
                      <span className="text-3xl font-bold tracking-tight">{task.count}</span>
                    </div>
                    <p className="mt-5 text-sm font-semibold">{index + 1}. {task.title}</p>
