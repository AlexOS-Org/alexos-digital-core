...
export default function TodaysMission() {
  ...
  const overdueBills = bills.filter(
    (bill) => bill.status === "pending" && bill.due_date && bill.due_date < today,
  ).length;
  ...
  const tasks = [
    {
      ...
      value: overdueBills,
      label: overdueBills ? "Needs attention" : "Clear",
      tone: "amber",
    },
    {
      title: "Expected payments today",
      value: expectedToday,
      label: expectedToday ? "Watch today" : "Nothing due",
      tone: "blue",
    },
    {
      title: "Active debts to monitor",
      value: activeDebts,
      label: activeDebts ? "Keep visible" : "All clear",
      tone: "violet",
    },
  ];
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
              AlexOS is surfacing the few things most likely to matter today.
            </p>
          </div>
          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-xs text-slate-300">
            ...
            {tasks.filter((t) => t.value > 0).length} active priorities
          </div>
        </div>
          ...
          {tasks.map((task) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div
                  className={`flex h-10 w-10 items-center justify-center rounded-xl ${task.value > 0 ? "bg-amber-400/10 text-amber-300" : "bg-emerald-400/10 text-emerald-300"}`}
                >
                  {task.value > 0 ? (
