...
export function AlexOSLogo({ showWordmark = false, compact = false, className, ...props }: AlexOSLogoProps) {
  return (
    <div className={`flex items-center gap-2 ${className ?? ""}`}>
      ...
      >
        ...
        <path d="M11 36 21 12h6l10 24h-6l-2.4-6H19.4L17 36h-6Zm10.6-11h3.8L23.5 20l-1.9 5Z" fill="url(#alexosLogoGradient)" />
        ...
        <path d="M35 10 41 17 33 22" fill="none" stroke="url(#alexosLogoGradient)" strokeWidth="1" opacity=".8" />
      </svg>
      {showWordmark && (
        <div className="min-w-0 leading-tight">
          <div className="truncate text-sm font-semibold tracking-tight">AlexOS</div>
          <div className="truncate text-[9px] font-medium uppercase tracking-[0.2em] text-sidebar-foreground/60">Business OS</div>
        </div>
      )}
    </div>
  );
