...
function categoryId(row) {
  return (
    categoryIds[row.subcategory] ?? categoryIds[row.category] ?? categoryIds["Deals & Featured"]
  );
}
...
function price(row) {
  const observed = [
    ...new Set(row.observed_prices_kes.filter((n) => Number.isFinite(n) && n >= 100)),
  ];
  return observed.length === 1 ? observed[0] : null;
}
function aida(row) {
  const proof =
    row.captions[0]?.replace(/\s+/g, " ").slice(0, 260) ||
    "Source post requires owner review for product facts.";
...
for (const row of plan.clusters) {
  const identity = row.image_urls[0]
    ? `${row.canonical_name}|${row.image_urls[0]}`
    : `${row.canonical_name}|${row.key}`;
...
fs.writeFileSync(
  "/home/ubuntu/alexos-source/notes/dailygear-draft-import-2026-08-22.sql",
  `-- Staged DailyGear social import. All rows remain Draft, zero-stock, availability unconfirmed.\n-- External image URLs only; no local product media is written.\nBEGIN;\n${statements.join("\n")}\nCOMMIT;\n`,
);
console.log(
  JSON.stringify({
    products: normalizedClusters.length,
    sql: "/home/ubuntu/alexos-source/notes/dailygear-draft-import-2026-08-22.sql",
  }),
);
