...
function s(v){return `'${String(v ?? '').replaceAll("'", "''")}'`;}
function slug(v){return v.toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,90);}
function j(v){return `${s(JSON.stringify(v))}::jsonb`;}
function variants(row){
  const text=row.captions.join(' ');
  const found=[];
  const colorMatch=text.match(/(?:colou?rs?|available in|available:|colou?r options?)\s*[:\-]?\s*([a-z][a-z ,/&-]{2,80})/i);
  if(colorMatch){
    for(const c of colorMatch[1].split(/[,/&]|\band\b/i).map(x=>x.trim().replace(/[.!]+$/,'')).filter(x=>/^[a-z][a-z -]{1,24}$/i.test(x))){
...
const out=['-- Social-import variants remain editable, zero-stock, and unavailable until owner verification.','BEGIN;'];
for(const row of plan.clusters){
  ...
  variants(row).forEach((opt,i)=>{
    ...
    const sku=`DG-VAR-${row.key.toUpperCase().replace(/[^A-Z0-9]+/g,'-').slice(0,34)}-${i+1}`;
...
out.push('COMMIT;');
fs.writeFileSync('/home/ubuntu/alexos-source/notes/dailygear-variant-import-2026-08-22.sql',out.join('\n')+'\n');
console.log(JSON.stringify({products:plan.clusters.length,variants:plan.clusters.reduce((n,r)=>n+variants(r).length,0)}));
