...
const files = fs
  ...
  .sort();
const priceRe = /(?:ksh|kes|price|selling|only|@)\s*\.?\s*([0-9][0-9,]*(?:\.[0-9]+)?)/gi;
const excludeRe = /bf\s*suma|bfsuma|veggie\s*veggie|dietary supplement|supplement/i;
const productSignalRe =
  /price|ksh|kes|watch|bag|shoe|table|wardrobe|charger|powerbank|blender|earphone|camera|sandal|wallet|pillow|suitcase|compressor|apron|backpack|sneaker|dress|cabinet|handbag|loafers|smart|car dent|tyre|duvet|privacy fence|organizer/i;
const posts = [];
...
const output = {
  source_account: "@daily_gearz",
  fetched_files: files,
  fetched_posts: result.length,
  excluded_count: result.filter((r) => r.excluded_from_dailygear).length,
  product_like_count: result.filter((r) => r.product_like && !r.excluded_from_dailygear).length,
  posts: result,
};
fs.writeFileSync(
...
console.log(
  JSON.stringify({
    fetched_posts: output.fetched_posts,
    excluded_count: output.excluded_count,
    product_like_count: output.product_like_count,
  }),
);
